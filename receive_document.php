                ...
                <tbody>
                    ...
                    try {
                        ...
                        foreach ($db->query($sql) as $row) {
                            $dateUpload = new DateTime($row['dateupload']);
                            $dateEnd = new DateTime($row['dateend']);
                            $currentTime = new DateTime();

                            if ($currentTime > $dateEnd) {
                                $row['expired'] = true;
                                $expiredDocuments[] = $row;
                            } else {
                                $row['expired'] = false;
                                $activeDocuments[] = $row;
                            }
                        ...
                        foreach ($expiredDocuments as $row) {
                            displayDocumentRow($row, $i); // Pass $i to the function
                            $i++;
                        }

                        foreach ($activeDocuments as $row) {
                            displayDocumentRow($row, $i); // Pass $i to the function
                            $i++;
                        }
                    ...
                    function displayDocumentRow($row, $counter) {
                        ...
                        $timeRemaining = $currentTime->diff($dateEnd);

                        $timeRemainingString = '';
                        if ($timeRemaining->days > 0) {
                            $timeRemainingString = $timeRemaining->format('%ad and %H:%I:%S');
                        } else {
                            $timeRemainingString = $timeRemaining->format('%H:%I:%S');
                        }

                        
                        // Check if the status is 'Open' or 'open' before displaying the row
